...
5. dim 4: 英雄位置 (0, 1) 分别为 (无, 有)
    ...
    def save_map(self):
        ...
        # print(f"{'='*10} [DEBUG] {np.sum(self.end > 0)=} {img[self.end > 0]}{'='*10}")
        if np.sum(self.end>0):
            img[self.end > 0] = [255, 0, 255] * self.end[self.end > 0]  # 终点位置 (pink)
        else:
            print(f"[WARNING] {self.step=} Not found end!")
...
class StateManager:
    ...
    def get_reward(
        ...
        if treasure_get > 0:
            ...
            self.collected_treasure += treasure_get
        # 6. buff奖励
        if (
        ...
        # ** 总宝箱数目为8, 宝箱全部收集完考虑终点j距离奖励 ** (弃用)
        # if self.end.pos[0] != -1 and self.collected_treasure == 8:  # 终点存在
    ...
    def save_frame(self):
        ...
        print(f"Finished saving frame info. {self.n_step=}")
