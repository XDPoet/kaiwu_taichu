...
class Agent(BaseAgent):
    ...
    def predict(self, list_obs_data):
        ...
        #     self.algorithm.model.load_state_dict(torch.load("/data/projects/back_to_the_realm_v2/ckpt/model.ckpt-1856.pkl", map_location=self.algorithm.device))
        #     self.logger.info(f"load model in first predict successfully")
        return self.algorithm.predict_detail(list_obs_data, exploit_flag=False)

    @exploit_wrapper
    def exploit(self, observation):
        obs_data, _ = self.observation_process(observation["obs"], extra_info=observation["extra_info"])
